...
class ExtendedCompatibilityTest {
    ...
    @Test
    fun `aws sdk kotlin path-style compatibility contract`() = runBlocking {
        newKotlinClient().use { s3 ->
            ...
            val head = s3.headObject(HeadObjectRequest {
                this.bucket = bucket
                key = "objects/hello.txt"
            })
            ...
            val got = s3.getObject(GetObjectRequest {
                this.bucket = bucket
                key = "objects/hello.txt"
            }) { response ->
            ...
            val missing = assertThrows<NoSuchKey> {
                runBlocking {
            ...
            val range = s3.getObject(GetObjectRequest {
                this.bucket = bucket
                key = "objects/range.bin"
                range = "bytes=10-19"
            }) { response ->
            ...
            val listed = s3.listObjectsV2(ListObjectsV2Request {
                this.bucket = bucket
                prefix = "prefix/"
                delimiter = "/"
            })
            ...
            val copied = s3.getObject(GetObjectRequest {
                this.bucket = bucket
                key = "objects/copied.txt"
            }) { response ->
                response.body?.toByteArray() ?: ByteArray(0)
            ...
            val init = s3.createMultipartUpload(CreateMultipartUploadRequest {
                this.bucket = bucket
                key = "multipart.bin"
            })
            val part = s3.uploadPart(UploadPartRequest {
                this.bucket = bucket
                key = "multipart.bin"
            ...
            assertEquals(1, s3.listParts(ListPartsRequest {
                this.bucket = bucket
                key = "multipart.bin"
                uploadId = init.uploadId
            }).parts?.size)
            ...
            s3.completeMultipartUpload(CompleteMultipartUploadRequest {
                ...
                multipartUpload = CompletedMultipartUpload {
                    parts = listOf(CompletedPart {
                        partNumber = 1
                        eTag = part.eTag
                    })
                }
            })
            assertEquals(PART_SIZE.toLong(), s3.headObject(HeadObjectRequest {
                this.bucket = bucket
                key = "multipart.bin"
            }).contentLength)

            val abort = s3.createMultipartUpload(CreateMultipartUploadRequest {
                this.bucket = bucket
                key = "abort.bin"
            })
            ...
            (java.net.URI.create(presignedPut.url.toString()).toURL().openConnection() as HttpURLConnection).apply {
                requestMethod = "PUT"
                doOutput = true
                outputStream.use { it.write("presigned".toByteArray()) }
                getInputStream().close()
            }
            val presigned = s3.getObject(GetObjectRequest {
                this.bucket = bucket
                key = "presigned-put.txt"
            }) { response ->
                response.body?.toByteArray() ?: ByteArray(0)
    ...
    private fun newBucket(client: String): String =
        "m10-$client-${UUID.randomUUID().toString().take(8).lowercase()}"

    private fun String.toUrlBytes(): ByteArray =
        java.net.URI.create(this).toURL().openStream().readBytes()
