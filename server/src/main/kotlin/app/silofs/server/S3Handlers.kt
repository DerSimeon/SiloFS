...
class S3Handlers(
    ...
    suspend fun listObjectsV2(
        ...
        val body =
            s3XmlDocument("ListBucketResult") {
                s3Tag("Name", bucket)
                s3Tag("Prefix", prefix ?: "")
    ...
    suspend fun putObject(
        ...
        // used. For aws-chunked SigV4 framing we require
        // x-amz-decoded-content-length and validate against decoded bytes.
    ...
    suspend fun copyObject(
        ...
        try {
            ...
            val body =
                s3XmlDocument("CopyObjectResult") {
    ...
    suspend fun deleteObjects(
        ...
        withS3 {
            config.database.withTransaction { conn ->
                if (!repo.bucketExists(conn, bucket)) throw S3Errors.noSuchBucket(bucket)
                request.objects.forEach { entry ->
                    try {
                        ObjectKey.validate(entry.key)
                        repo.deleteObject(conn, bucket, entry.key)
                        deleted += entry.key
                    } catch (e: S3Exception) {
                        errors += entry.key to e
                    }
                }
            }
        }
        ...
        val xml =
            s3XmlDocument("DeleteResult") {
                ...
                errors.forEach { (key, error) ->
                    s3Open("Error")
                    s3Tag("Key", key)
                    s3Tag("Code", error.errorCode.code)
                    s3Tag("Message", error.message)
                    s3Close("Error")
                }
            }
    ...
    private fun appendChecksumHeaders(
        call: ApplicationCall,
        meta: ObjectMetadata,
    ) {
        call.response.headers.apply {
    ...
    companion object {
        ...
        val ALLOWED_STORAGE_CLASSES: Set<String> =
            setOf(
                "STANDARD",
                "REDUCED_REDUNDANCY",
                "GLACIER",
