...
class MultipartHandlers(
    ...
    suspend fun createMultipartUpload(
        ...
        if (effectiveStorageClass !in S3Handlers.ALLOWED_STORAGE_CLASSES) {
            throw S3Errors.invalidArgument(
                "x-amz-storage-class",
                effectiveStorageClass,
                "must be one of ${S3Handlers.ALLOWED_STORAGE_CLASSES}",
            )
        }
        ...
        val body =
            s3XmlDocument("InitiateMultipartUploadResult") {
                s3Tag("Bucket", bucket)
                s3Tag("Key", key)
    ...
    suspend fun uploadPart(
        ...
        try {
            ...
            log.debug(
                "UploadPart bucket={} key={} uploadId={} part={} size={}",
                bucket,
                key,
                uploadId,
                partNumber,
    ...
    suspend fun completeMultipartUpload(
        ...
        try {
            ...
            try {
                ...
                log.info(
                    "CompleteMultipartUpload bucket={} key={} uploadId={} parts={} size={}",
                    bucket,
                    key,
                    uploadId,
                    orderedParts.size,
                ...
                val body =
                    s3XmlDocument("CompleteMultipartUploadResult") {
    ...
    suspend fun listMultipartUploads(
        ...
        val body =
            s3XmlDocument("ListMultipartUploadsResult") {
                s3Tag("Bucket", bucket)
    ...
    suspend fun uploadPartCopy(
        ...
        try {
            ...
            log.debug(
                "UploadPartCopy dest={}/{} uploadId={} part={} src={}/{} range={}",
                destBucket,
                destKey,
                uploadId,
                partNumber,
    ...
    private fun createBlobWriteIntent(blobSha256Hex: String): String =
        withS3 {
    ...
    private fun normalizeEtag(etag: String): String = etag.trim().removeSurrounding("\"")
