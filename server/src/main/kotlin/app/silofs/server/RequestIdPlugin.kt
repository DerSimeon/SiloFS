...
val RequestIdPlugin: ApplicationPlugin<RequestIdConfig> =
    createApplicationPlugin(
        ...
        application.intercept(ApplicationCallPipeline.Setup) {
            ...
            context.response.headers.apply {
                ...
                append(
                    HttpHeaders.Date,
                    app.silofs.common.S3Time
                        .formatHttpDate(Instant.now()),
                )
...
fun ApplicationCall.requestId(): String? = attributes.getOrNull(callRequestIdKey)

/** Read the per-call extended ID, or null. */
fun ApplicationCall.extendedRequestId(): String? = attributes.getOrNull(callExtendedIdKey)
