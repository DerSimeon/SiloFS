...
fun Application.s3Routes(
    ...
    routing {
        ...
        route("/{bucket}") {
            ...
            post {
                val bucket = call.parameters["bucket"]!!
                handleBucketPost(call, config, handlers, bucket)
            }
            get {
                val bucket = call.parameters["bucket"]!!
                when {
...
private suspend fun handleBucketPost(
    call: ApplicationCall,
    config: ServerConfig,
    handlers: S3Handlers,
    bucket: String,
) {
    if (call.request.queryParameters["delete"] != null) {
        val body = call.receiveBoundedText(config.operationalConfig.completeXmlMaxBytes)
        handlers.deleteObjects(call, bucket, parseDeleteObjectsXml(body))
    } else {
        throw S3Errors.notImplemented("POST bucket operation without ?delete")
    }
}
...
internal fun parseCompleteMultipartUploadXml(body: String): List<RequestedPart> {
    ...
    try {
        ...
        reader.errorHandler =
            object : org.xml.sax.helpers.DefaultHandler() {
                override fun error(e: org.xml.sax.SAXParseException): Unit =
                    throw S3Errors.malformedXML("XML parse error at line ${e.lineNumber}: ${e.message}")

                override fun fatalError(e: org.xml.sax.SAXParseException): Unit =
                    throw S3Errors.malformedXML("XML parse error at line ${e.lineNumber}: ${e.message}")
            }
...
private fun validateObjectEncryptionHeaders(
    call: ApplicationCall,
    config: ServerConfig,
) {
