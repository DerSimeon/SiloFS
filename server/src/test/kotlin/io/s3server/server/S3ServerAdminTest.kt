...
class S3ServerAdminTest {
    ...
    private fun ServerConfig.copyForCommand(): ServerConfig =
        copy(database = Database.fromUrl(pg.jdbcUrl, pg.username, pg.password))

    private fun seedObject(config: ServerConfig) {
        val stored = config.blobStore.beginWrite().use { write ->
