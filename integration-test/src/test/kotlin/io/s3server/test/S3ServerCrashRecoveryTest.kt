        ...
        try {
            ...
            val config1 = ServerConfig(
                ...
                recoveryConfig = app.silofs.server.RecoveryConfig(
                    tempMaxAgeSeconds = 1, multipartMaxAgeSeconds = 1,
                    sweepIntervalSeconds = 1, blobSweepIntervalSeconds = 1,
                    enabled = false
                )
            )
            val server1 = embeddedServer(Netty, host = "127.0.0.1", port = 0) {
                s3Module(config1)
            }
            server1.start(wait = false)
            val port1 = runBlocking { server1.engine.resolvedConnectors().first().port }
            val endpoint1 = "http://127.0.0.1:$port1"

            val payload = "durable-data".toByteArray()
            val bucket = "crash-" + UUID.randomUUID().toString().take(8).lowercase()

            val s3 = software.amazon.awssdk.services.s3.S3Client.builder()
            ...
            s3.use {
                ...
                it.putObject(
                    PutObjectRequest.builder().bucket(bucket).key("k.bin").build(),
            ...
            val server2 = embeddedServer(Netty, host = "127.0.0.1", port = 0) {
                s3Module(config2)
            }
            server2.start(wait = false)
            val port2 = runBlocking { server2.engine.resolvedConnectors().first().port }
            val endpoint2 = "http://127.0.0.1:$port2"

            val s3b = software.amazon.awssdk.services.s3.S3Client.builder()
                .region(software.amazon.awssdk.regions.Region.of(REGION))
                .credentialsProvider(software.amazon.awssdk.auth.credentials.StaticCredentialsProvider.create(
                    software.amazon.awssdk.auth.credentials.AwsBasicCredentials.create(ACCESS_KEY, SECRET_KEY)
                ))
