...
class S3ServerM2Test : AbstractS3ServerTest() {
    ...
    fun `put get head metadata roundtrip`() {
        newS3Client().use { s3 ->
            ...
            assertEquals(
                "attachment; filename=\"meta.txt\"",
                getResp.sdkHttpResponse().firstMatchingHeader("Content-Disposition").orElse(null),
            )
        ...
        newS3Client().use { s3 ->
            ...
            val lmHeader =
                get
                    .response()
                    .sdkHttpResponse()
                    .firstMatchingHeader("Last-Modified")
                    .orElse(null)
        ...
        newS3Client().use { s3 ->
            ...
            assertEquals(
                ...
                get
                    .response()
                    .sdkHttpResponse()
                    .firstMatchingHeader("Accept-Ranges")
                    .orElse(null),
            )
            val head = s3.headObject { it.bucket(bucket).key("k") }
    ...
    @Test
    fun `delete objects removes requested keys and reports missing keys as deleted`() {
        newS3Client().use { s3 ->
            val bucket = newBucket()
            s3.createBucket { it.bucket(bucket) }
            listOf("a.txt", "b.txt").forEach { key ->
                s3.putObject(
                    PutObjectRequest
                        .builder()
                        .bucket(bucket)
                        .key(key)
                        .build(),
                    RequestBody.fromBytes(key.toByteArray()),
                )
            }
            ...
            assertTrue(response.deleted().map { it.key() }.containsAll(listOf("a.txt", "b.txt", "missing.txt")))
            assertThrows<NoSuchKeyException> {
                s3.headObject { it.bucket(bucket).key("a.txt") }
            }
            assertThrows<NoSuchKeyException> {
                s3.headObject { it.bucket(bucket).key("b.txt") }
            }
        }
    }
