...
class ETagTest {
    ...
    fun `multipart etag rejects empty part list`() {
        org.junit.jupiter.api.assertThrows<IllegalArgumentException> { ETag.fromMultipart(emptyList()) }
    }

    @Test
    fun `multipart etag rejects wrong-size md5`() {
        org.junit.jupiter.api.assertThrows<IllegalArgumentException> { ETag.fromMultipart(listOf(ByteArray(15))) }
    }
