...
object ETag {
    ...
    fun fromMultipart(partMd5s: List<ByteArray>): String {
        ...
        val digest = java.security.MessageDigest.getInstance("MD5").digest(concatenated)
...
object Sha256 {
    fun hex(bytes: ByteArray): String =
        bytes.joinToString("") { "%02x".format(it) }.lowercase()
 ...
 */
object Md5Base64 {
    fun encode(md5: ByteArray): String = java.util.Base64.getEncoder().encodeToString(md5)
    fun decode(b64: String): ByteArray = java.util.Base64.getDecoder().decode(b64)
}
