...
object S3Errors {
    fun noSuchBucket(bucket: String): S3Exception =
        S3Exception(S3ErrorCode.NoSuchBucket, resource = "/$bucket")

    fun noSuchKey(bucket: String, key: String): S3Exception =
        S3Exception(S3ErrorCode.NoSuchKey, resource = "/$bucket/$key")
    ...
    fun invalidRange(size: Long, rangeHeader: String): S3Exception =
        S3Exception(
    ...
    fun signatureDoesNotMatch(reason: String): S3Exception =
        S3Exception(S3ErrorCode.SignatureDoesNotMatch, "The request signature we calculated does not match the signature you provided. $reason")
    ...
    fun invalidArgument(name: String, value: String?, reason: String): S3Exception =
        S3Exception(S3ErrorCode.InvalidArgument, "Invalid argument: $name='$value' — $reason")

    fun badDigest(expected: String, actual: String): S3Exception =
        S3Exception(S3ErrorCode.BadDigest, "The Content-MD5 you specified did not match what we received. expected=$expected actual=$actual")

    fun sha256Mismatch(expected: String, actual: String): S3Exception =
        S3Exception(S3ErrorCode.XAmzContentSHA256Mismatch, "The provided 'x-amz-content-sha256' header does not match what was computed. expected=$expected actual=$actual")
    ...
    fun invalidAccessKeyId(accessKeyId: String): S3Exception =
        S3Exception(S3ErrorCode.InvalidAccessKeyId, "The AWS Access Key Id you provided does not exist in our records. accessKeyId=$accessKeyId")
    ...
    fun notModified(): S3Exception =
        S3Exception(S3ErrorCode.NotModified, "Not Modified")

    fun requestRangeNotSatisfiable(size: Long, rangeHeader: String): S3Exception =
        S3Exception(
    ...
    fun malformedXML(reason: String): S3Exception =
        S3Exception(S3ErrorCode.MalformedXML, "The XML you provided was not well-formed or did not validate against our published schema. $reason")

    fun invalidArgumentList(name: String, value: String?, reason: String): S3Exception =
        S3Exception(S3ErrorCode.InvalidArgument, "Invalid argument: $name='$value' — $reason")

    fun methodNotAllowed(method: String, resource: String): S3Exception =
        S3Exception(S3ErrorCode.MethodNotAllowed, "The specified method is not allowed against this resource. method=$method resource=$resource")
    ...
    fun noSuchPart(uploadId: String, partNumber: Int): S3Exception =
        S3Exception(
            S3ErrorCode.NoSuchPart,
            "The specified part does not exist. uploadId=$uploadId partNumber=$partNumber"
        )

    fun invalidPartETag(partNumber: Int, expected: String, actual: String): S3Exception =
        S3Exception(
            S3ErrorCode.InvalidPart,
            "The ETag for part $partNumber does not match the stored ETag. expected=$expected actual=$actual"
        )

    fun entityTooSmall(partNumber: Int, actual: Long, min: Long): S3Exception =
        S3Exception(
    ...
    fun invalidPart(partNumber: Int, reason: String): S3Exception =
        S3Exception(
            S3ErrorCode.InvalidPart,
    ...
    fun incompleteBody(declared: Long, actual: Long): S3Exception =
        S3Exception(
