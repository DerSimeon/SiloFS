...
fun s3XmlDocument(rootTag: String, body: StringBuilder.() -> Unit): String {
...
object S3Xml {
    fun document(rootTag: String, body: StringBuilder.() -> Unit): String =
        s3XmlDocument(rootTag, body)

    fun escape(s: String): String = s3XmlEscape(s)
    fun escapeAttr(s: String): String = s3XmlEscape(s)
}

fun StringBuilder.s3Tag(name: String, value: String?) {
...
fun StringBuilder.s3TagLong(name: String, value: Long?) {
    if (value == null) return
    append('<').append(name).append('>').append(value).append("</").append(name).append('>')
}

fun StringBuilder.s3TagBool(name: String, value: Boolean) {
    append('<').append(name).append('>').append(if (value) "true" else "false").append("</").append(name).append('>')
}

fun StringBuilder.s3Open(name: String, attrs: Map<String, String> = emptyMap()) {
    append('<').append(name)
    attrs.forEach { (k, v) -> append(' ').append(k).append("=\"").append(s3XmlEscape(v)).append('"') }
    append('>')
}
