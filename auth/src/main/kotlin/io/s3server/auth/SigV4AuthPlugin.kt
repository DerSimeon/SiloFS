...
object SigV4CanonicalBuilder {
    ...
    fun canonicalQueryString(query: String): String {
        ...
        val pairs = query.split('&').mapIndexedNotNull { idx, kv ->
    ...
    fun canonicalHeaders(headers: Map<String, List<String>>, signedHeaders: List<String>): String {
    ...
    fun build(req: SignedHttpRequest, auth: SigV4Authorization): CanonicalRequest {
...
val SigV4Auth: ApplicationPlugin<SigV4AuthConfig> = createApplicationPlugin(
    ...
    application.intercept(ApplicationCallPipeline.Setup) {
        ...
        val amzDate = context.request.headers["x-amz-date"]
 ...
 */
private fun validateClockSkew(amzDate: String, maxSkewSeconds: Long) {
    val signedInstant = try {
        parseAmzDate(amzDate)
    } catch (e: Exception) {
        throw S3Errors.authorizationHeaderMalformed("Invalid x-amz-date format: $amzDate")
    }
    val now = java.time.Instant.now()
    val skew = java.time.Duration.between(signedInstant, now).abs().seconds
...
private fun verifyPresignedUrl(
    ...
    val algorithm = q["X-Amz-Algorithm"]
    ...
    val credential = q["X-Amz-Credential"]
    ...
    val queryStringWithoutSig = context.request.queryString()
        .split("&")
    ...
    val ok = SigV4Verifier.verify(
        canonicalRequest, amzDate,
        SigV4Authorization(
            algorithm = algorithm,
            accessKeyId = akid,
            date = dateStamp,
