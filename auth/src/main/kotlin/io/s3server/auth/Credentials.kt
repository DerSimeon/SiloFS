...
class StaticCredentialProvider(private val creds: Map<String, AwsCredentials>) : CredentialProvider {
    override fun lookup(accessKeyId: String): AwsCredentials? = creds[accessKeyId]
    companion object {
        fun single(accessKeyId: String, secretAccessKey: String): StaticCredentialProvider =
            StaticCredentialProvider(mapOf(accessKeyId to AwsCredentials(accessKeyId, secretAccessKey)))
    }
}
