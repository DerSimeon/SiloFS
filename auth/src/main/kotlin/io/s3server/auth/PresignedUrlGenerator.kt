...
class PresignedUrlGenerator(
    ...
    fun generate(
        ...
        val canonicalQueryString = params.entries
            .sortedBy { it.key }
            .joinToString("&") { (k, v) ->
                "${encode(k)}=${encode(v)}"
            }
        ...
        val sig = HmacSha256.hmac(signingKey, stringToSign.toByteArray(Charsets.UTF_8))
    ...
    /** Convenience for GET. */
    fun presignedGet(host: String, path: String, expires: Duration = Duration.ofHours(1)): String =
        generate("GET", host, path, expires)

    /** Convenience for PUT. */
    fun presignedPut(host: String, path: String, expires: Duration = Duration.ofHours(1)): String =
        generate("PUT", host, path, expires)
